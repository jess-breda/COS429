...
def loss_crossentropy(input, labels, hyper_params, backprop):
    ...
    # take log of activity for a target label for a batch, normalize by bactch size
    ...
    if backprop:
        ...
        dv_input[labels, np.arange(batch_size)] = -1 / (
            input[labels, np.arange(batch_size)] + eps
        )
